...
async function generateLoginEncryptedData(username, password) {
  console.log('Using real dynamic encryption for login with username:', username);
  ...
  try {
    ...
    console.log('AES Key WordArray:', aesKeyWordArray.toString());
    console.log('Base64 AES Key:', base64AesKey);
...
async function loginWithNewAPI(username, password) {
  try {
    ...
    console.log('Attempting login with new API...')
    console.log('Request URL:', LOGIN_API)
...
async function testToken(token) {
  try {
    ...
    const response = await axios.request(config)
    console.log('Token test response:', JSON.stringify(response.data, null, 2))
